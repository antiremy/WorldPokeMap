...
map = L.map('map',{minZoom:3, tap: false, worldCopyJump: true});
var mapboxTiles = L.tileLayer(server + style + '/{z}/{x}/{y}.png').addTo(map)
//var mapboxTiles = L.tileLayer('https://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png').addTo(map)
map.on('load', function(e){search();});
map.setView([19.311143355064655, -3.5156250000000004], 3);
map.on('zoomend dragend',function(e){lastTime=null;search();});
...
function makePokemon(pokemon){
  ...
  pokemon.forEach(function(poke,index) {
    ...
    var icon = L.icon ({
      iconUrl: 'images/' + pokeID +'.png',
      iconSize: [$("#iconSize").val(),$("#iconSize").val()]
    })
    var popup
    if (attack != null) {
      popup = L.popup()
        .setContent('<strong>'+name+'</strong> - ' + pokeNames[pokeID]['rarity'] + '<br>' +
        'Disappeared At: ' + moment(dt).format() + '<br>' +
        '<strong>IV: ' + iv+ '% (' + attack + '/' + defense + '/' + stamina + ')</strong><br>'
    ...
    marker.on('popupclose', function(e) {
      ...
      // if (pokeNames[pokeID]['rarity'] == 'Common' && !commonID.includes(encID)) {
      //   marker.addTo(common)
      //   commonID.push(encID)
      //   commonMarkers.push(marker)
      // }
      // else if (pokeNames[pokeID]['rarity'] == 'Uncommon' && !uncommonID.includes(encID)) {
      //   marker.addTo(uncommon)
      //   uncommonID.push(encID)
      //   uncommonMarkers.push(marker)
      // }
      // else if (pokeNames[pokeID]['rarity'] == 'Rare' && !rareID.includes(encID)) {
      //   marker.addTo(rare)
      //   rareID.push(encID)
      //   rareMarkers.push(marker)
      // }
      // else if (pokeNames[pokeID]['rarity'] == 'Very Rare' && !veryRareID.includes(encID)) {
      //   marker.addTo(veryRare)
      //   veryRareID.push(encID)
      //   veryRareMarkers.push(marker)
      // }
      // else if (pokeNames[pokeID]['rarity'] == 'ultraRare' && !ultraRareID.includes(encID)) {
...
function search() {
  ...
  "&pid=" + pid
  if (lastTime != null) {
    dataString += "&time=" + lastTime
  }
  if (!commonToggle) {
    dataString += "&common=False"
  }
  if (!uncommonToggle) {
    dataString += "&uncommon=False"
  }
  if (!rareToggle) {
    dataString += "&rare=False"
  }
  if (!veryRareToggle) {
    dataString += "&veryrare=False"
  }
  if (!ultraRareToggle) {
    dataString += "&ultrarare=False"
  }
  if (map.getZoom() < 12) {
    $.ajax({
      type : "POST",
      url : "statsCluster.php",
      data: dataString,
      cache: false,
...
$("#pokeIDChoice").change(function() {
  pid = $('#pokeIDChoice').val()
  clearAllPokemon()
  search()
})
