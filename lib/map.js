...
function formatPoke (poke) {
  if (!poke.id) { console.log(poke.text);return poke.text; }
  var htmlstring = $(
...
function makePokemon(pokemon){
  ...
  pokemon.forEach(function(poke,index) {
    ...
    if ($.inArray(pokeID,hidePokemon) > -1 || Date.parse(dt) < Date.now()) {
    ...
    if (attack != null) {
      popup = L.popup()
        ...
        '<br><span class="label-countdown" disappears-at=\'' + dt + '\'>' + getTimeRemaining(dt, "string")
        + '</span><br><strong>IV: ' + iv+ '% (' + attack + '/' + defense + '/' + stamina + ')</strong>'
