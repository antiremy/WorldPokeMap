...
function makePokemon(pokemon){
  ...
  pokemon.forEach(function(poke,index) {
    ...
    if (pokeNames[pokeID]['rarity'] == 'Common' && !commonMarkers.includes(encID) && commonToggle) {
      marker.addTo(common)
      commonMarkers.push(encID)
    }
    else if (pokeNames[pokeID]['rarity'] == 'Uncommon' && !uncommonMarkers.includes(encID) && uncommonToggle) {
      marker.addTo(uncommon)
      uncommonMarkers.push(encID)
    }
    else if (pokeNames[pokeID]['rarity'] == 'Rare' && !rareMarkers.includes(encID) && rareToggle) {
      marker.addTo(rare)
      rareMarkers.push(encID)
    }
    else if (pokeNames[pokeID]['rarity'] == 'Very Rare' && !veryRareMarkers.includes(encID) && veryRareToggle) {
      marker.addTo(veryRare)
      veryRareMarkers.push(encID)
    }
    else if (pokeNames[pokeID]['rarity'] == 'ultraRare' && !ultraRareMarkers.includes(encID) && ultraRareToggle) {
      marker.addTo(ultraRare)
      ultraRareMarkers.push(encID)
    }
...
function search() {
  $('.label-countdown').each(function (index, element) {
    $(element).textContent = (getTimeRemaining(element.getAttribute('disappears-at')))
  })
  var b = map.getBounds();
  var b1 = {
...
function getTimeRemaining(endtime){
  ...
  return 'Disappears in ' + hours + 'h' + minutes + 'm' + seconds + 's'
}
