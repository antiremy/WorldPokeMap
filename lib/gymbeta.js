...
function defaultInterval() {
  ...
  gymInterval = window.setInterval(searchGyms,60000)
}
...
function makeGyms(result) {
  ...
  gyms.forEach(function (gym, index) {
    ...
    popup +=    "Level: " + gymLevel + " | Prestige: " + gymPoints + "/" + prestigeMax
    if (typeof details != "undefined") {
      popup += "<div style='width=100%;'>"
      var pokemon = details['pokemon']
      pokemon.forEach(function(poke) {
        popup += "<span><img width='30' height='23' src='pixel_icons/" + poke['pokemon_id'] + ".png'><span class='cp team-" + teamID + "'>" + poke['cp'] + "</div></span></div>"
      })
      popup += "<br>"
    }
    ...
    popup += "</center>"
    leafletPopup = L.popup({className:"gymPopup"}).setContent(popup)
    var mouseOut = true
    var marker = L.marker([location[1], location[0]], {icon: icon})
